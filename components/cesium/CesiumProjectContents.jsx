...
export default class CesiumProjectContents extends Component {
   constructor(props) {
      super(props);
   ...
   render() {
      ...
      const renderedLabels = labels.map((label, index) =>
         <CesiumLabel {...label} labels={this.labels} key={index} />
      );

      const renderedPolylines = polylines.map((polyline, index) =>
         <CesiumPolyline coords={polyline} polylines={this.polylines} key={index} />
      );
